...
下载 [Komari-Glass-Kv-3.3.3-kv.3.zip](./Komari-Glass-Kv-3.3.3-kv.3.zip)，
...
这些结构化标签会进入卡片对应位置，不会再作为普通标签重复显示。三网五格短柱沿用主题现有阈值，分别对延迟和丢包分级，并采用两者中较差的等级；底部综合短柱仍来自真实历史记录。
