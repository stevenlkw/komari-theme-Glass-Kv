...
- 不做：不删除生产组件和路由，不修改 Komari 核心数据模型，不新增依赖或重复请求。
- 验证：Bun 1.3.14 下 `bun run lint`、`bun run build`、`git diff --check` 通过；Vue 严格类型检查通过；Impeccable 机械检测为零告警；归档包含 manifest、preview.png、dist/index.html。
- 成品：`Komari-Glass-Kv-3.3.3-kv.6.zip`，manifest 版本 3.3.3-kv.6，SHA-256 为 `396876796B24B0CF2F106980372E19000887685E12F3F3956189443E48D6F705`。

- 状态：done，Kv v3.3.3-kv.5 卡片资料行与服务商识别修复完成
- 目标：避免带宽/三网结构化标签污染服务商自动识别；移除虚拟化项，并清晰展示后台已设置的金额和付费周期。
- 实现：服务商优先显示后台分组；结构化标签从共享服务商识别输入中排除；移除 `kvm`；价格与周期使用独立彩色标签。
- 不做：不改 IP 识别、分组数据、价格数据、权限或请求逻辑；价格仍尊重原有的未登录隐藏设置。
- 验证：Bun 1.3.14 下 `bun run lint`、`bun run build`、`git diff --check` 通过；Impeccable 机械检测为零告警；归档包含 manifest、preview.png、dist/index.html。
- 成品：`Komari-Glass-Kv-3.3.3-kv.5.zip`，manifest 版本 3.3.3-kv.5，SHA-256 为 `F9EA1F3B43CF3351081FF2CBB30B317EA5F6B91419F7BE9055CF99EFCDD6C85D`。

- 状态：done，Kv v3.3.3-kv.4 三网短柱语义修复完成
- 目标：消除三网行五格短柱被误解为网速的问题，同时保留真实的延迟与丢包综合判断。
- 实现：线路图标替换为路由；短柱新增“质量”文字与风险色；丢包改用百分比图标并随丢包风险着色；悬浮说明明确短柱不是网速。
- 不做：不改 Ping 数据、任务识别、历史图、既有延迟/丢包阈值或请求路径。
- 验证：Bun 1.3.14 下 `bun run lint`、`bun run build`、`git diff --check` 通过；Impeccable 机械检测为零告警；归档包含 manifest、preview.png、dist/index.html。
- 成品：`Komari-Glass-Kv-3.3.3-kv.4.zip`，manifest 版本 3.3.3-kv.4，SHA-256 为 `3EF9DC910FED22905AC5C53BFA4793DAD98CEB430B7B5277EC044074BE127EC8`。
